...
public class Life extends Poly {
    ...
    for (int tr = -r; tr <= r; tr++)
      for (int tc = -r; tc <= r; tc++)
        for (int row = 0; row < boardHeight; row++) {
          for (int col = 0; col < boardWidth; col++) {
            if (board[row][col] != -1) {
              float newC = v.hueShift(board[row][col] + tr + tc);
              v.fill(newC, 255, 255, v.alp);
              v.strokeWeight(1);
              v.stroke(v.hueShift(90), 255, 255, 255);
              // v.fill(v.hueShift(newC),(255+v.frameCount)%255,(255+v.frameCount)%255,
              // v.alp);

              // v.stroke(v.hueShift(newC + 127), 255, 255, v.alp);
              // v.strokeWeight(2);
              v.pushMatrix();
              v.translate(tc * col * cellWidth, tr * row * cellWidth, tc);
              v.box(cellWidth);
              v.popMatrix();
  ...
  public void render() {
    ...
    v.camera(0, 0, v.pal, 0, 0, 0f, 0f, 0.001f, 0f);
    v.rotateX(v.pit);
    v.rotateY(-v.yaw);
    v.rotateZ(v.rol);
    ...
    float toPass = v.map(v.spe, 0, 3.58f, 1.0f, 0.1f);
